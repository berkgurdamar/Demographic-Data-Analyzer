...
def calculate_demographic_data(print_data=True):
    ...
    highest_earning_country = perc[perc == perc.max()].index[0]
    highest_earning_country_percentage = round(perc.max(), 1)

    # Identify the most popular occupation for those who earn >50K in India.
    dff = df.loc[(df["native-country"] == "India") & (df["salary"] == ">50K"), "occupation"]
    dff_counts = dff.value_counts()
    top_IN_occupation = dff_counts[dff_counts == dff_counts.max()].index[0]
